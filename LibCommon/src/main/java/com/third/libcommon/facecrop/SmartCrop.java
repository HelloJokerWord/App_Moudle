package com.third.libcommon.facecrop;
...
public class SmartCrop {
    ...
    public CropResult analyze(Bitmap input) {
        ...
        if (topCrop != null) {
            result = CropResult.newInstance(topCrop, crops, output, createCrop(input, topCrop));
    ...
    private void faceDetect(Image inputI, Image outputI) {
        ...
        if (faceDetector.findFaces(inputI.bitmap, faces) > 0) {
            ...
            for (FaceDetector.Face face : faces)
                if (face != null) maxEyeDistance = Math.max(maxEyeDistance, face.eyesDistance());
    ...
    private float importance(Crop crop, int x, int y) {
        if (crop.x > x || x >= crop.x + crop.width || crop.y > y || y >= crop.y + crop.height) return options.getOutsideImportance();
